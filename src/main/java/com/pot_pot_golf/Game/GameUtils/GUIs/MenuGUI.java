...
import com.pot_pot_golf.Core.Entity.Terrain.*;
import com.pot_pot_golf.Core.GolfBots.AIBot;
import com.pot_pot_golf.Core.GolfBots.RuleBasedBot;
...
/** The menu GUI class. This class is responsible for creating the menu GUI of the game. */
public class MenuGUI {
    private record MenuRunnable(
    ...
    private void createMenu() {
        ...
        Button soundButton =
                new Button(
                        window.getWidth() - window.getWidthConverted(450),
                        window.getHeightConverted(20),
                        window.getWidthConverted(400),
                        heightButton,
                        "Sound: " + (gameStateManager.isSoundPlaying() ? "ON" : "OFF"),
                        font,
                        runnable.sound(),
                        vg,
                        imageButton,
        ...
        Button botButton =
                new Button(
                        window.getWidthConverted(30),
                        window.getHeight() - heightButton * 2,
                        widthButton / 4,
                        heightButton,
                        "Play with bot",
                        font,
                        gameStateManager::switchBot,
                        vg,
                        imageButton,
                        Consts.GUI.FONT);
        ...
        Button aiBotButton =
                new Button(
                        window.getWidthConverted(30),
                        window.getHeight() - heightButton * 3,
                        widthButton / 4,
                        heightButton,
                        "Play with AI",
                        font,
                        gameStateManager::switchAiBot,
                        vg,
                        imageButton,
                        Consts.GUI.FONT);
        ...
        Button twoPlayerButton =
                new Button(
                        window.getWidthConverted(30),
                        window.getHeight() - heightButton * 4,
                        widthButton / 4,
                        heightButton,
                        "2 Player",
                        font,
                        gameStateManager::switch2player,
                        vg,
                        imageButton,
                        Consts.GUI.FONT);
    ...
    private @NotNull MenuRunnable getRunnable() {
        Runnable terrainChanger =
                () -> {
                    try {
                        ...
                        if (entitiesManager.getBotBall() != null)
                            scene.getEntities()
                                    .removeIf(
                                            entity -> entity.equals(entitiesManager.getBotBall()));
                        if (entitiesManager.getAiBotBall() != null)
                            scene.getEntities()
                                    .removeIf(
                                            entity ->
                                                    entity.equals(entitiesManager.getAiBotBall()));

                        TerrainTexture blendMap2 =
                                new TerrainTexture(loader.loadTexture(Consts.HEIGHTMAP));
                        SimplexNoise.shufflePermutation();
                        terrainSwitch.terrainSwitch(blendMapTerrain, blendMap2);
                        entitiesManager.setGolfBallPosition(
                                new Vector3f(pathManager.getStartPoint()));
                        if (gameStateManager.is2player()) {
                            entitiesManager.setGolfBall2Position(
                                    new Vector3f(pathManager.getStartPoint()));
                        }
        ...
        Runnable startGame =
                () -> {
                    ...
                    if (entitiesManager.getBotBall() != null)
                        scene.getEntities()
                                .removeIf(entity -> entity.equals(entitiesManager.getBotBall()));
                    if (entitiesManager.getAiBotBall() != null)
                        scene.getEntities()
                                .removeIf(entity -> entity.equals(entitiesManager.getAiBotBall()));
                    ...
                    start.y =
                            heightMap.getHeight(
                                    new Vector3f(
                                            pathManager.getStartPoint().x,
                                            0,
                                            pathManager.getStartPoint().z));

                    Vector3f end = new Vector3f(pathManager.getEndPoint());
                    end.y =
                            heightMap.getHeight(
                                    new Vector3f(
                                            pathManager.getEndPoint().x,
                                            0,
                                            pathManager.getEndPoint().z));

                    if (gameStateManager.is2player()) {
                        if (entitiesManager.getGolfBall2() == null) {
                            Entity golfBall2 =
                                    new Entity(
                                            modelManager.getBall2(),
                                            new Vector3f(start),
                                            new Vector3f(0, 0, 0),
                                            5);
                        ...
                        if (entitiesManager.getGolfBall2() != null) {
                            scene.getEntities()
                                    .removeIf(
                                            entity ->
                                                    entity.equals(entitiesManager.getGolfBall2()));
                ...
                () -> {
                    ...
                    if (debugMode) {
                        ...
                        if (Consts.USE_PREDEFINED_POSITIONS && debugMode) {
                            Vector3f endPoint =
                                    new Vector3f(
                                            Consts.HOLE_POSITION.x,
                                            heightMap.getHeight(
                                                    new Vector3f(
                                                            Consts.HOLE_POSITION.x,
                                                            0,
                                                            Consts.HOLE_POSITION.z)),
                                            Consts.HOLE_POSITION.z);
                            Vector3f startPoint =
                                    new Vector3f(
                                            Consts.TEE_POSITION.x,
                                            heightMap.getHeight(
                                                    new Vector3f(
                                                            Consts.TEE_POSITION.x,
                                                            0,
                                                            Consts.TEE_POSITION.z)),
                                            Consts.TEE_POSITION.z);
                            ...
                            start.x =
                                    (int)
                                            ((start.x + Consts.SIZE_X / 2)
                                                    * (Consts.VERTEX_COUNT / Consts.SIZE_X));
                            start.y =
                                    (int)
                                            ((start.y + Consts.SIZE_Z / 2)
                                                    * (Consts.VERTEX_COUNT / Consts.SIZE_X));

                            Vector2i end = new Vector2i((int) endPoint.x, (int) endPoint.z);
                            end.x =
                                    (int)
                                            ((end.x + Consts.SIZE_X / 2)
                                                    * (Consts.VERTEX_COUNT / Consts.SIZE_X));
                            end.y =
                                    (int)
                                            ((end.y + Consts.SIZE_Z / 2)
                                                    * (Consts.VERTEX_COUNT / Consts.SIZE_X));
                            ...
                            List<Vector2i> path =
                                    pathfinder.getPathDebug(
                                            start, end, Consts.SIZE_GREEN, Consts.HEIGHTMAP);
                            pathManager.setPath(path);
                            try {
                                TerrainTexture blendMap2 =
                                        new TerrainTexture(loader.loadTexture(Consts.HEIGHTMAP));
                        ...
                        entitiesManager.setGolfBallPosition(
                                new Vector3f(
                                        startPoint.x,
                                        heightMap.getHeight(
                                                new Vector3f(startPoint.x, 0, startPoint.z)),
                                        startPoint.z));
                        if (gameStateManager.is2player()) {
                            entitiesManager.setGolfBall2Position(
                                    new Vector3f(
                        ...
                        entitiesManager.setEndFlagPosition(
                                new Vector3f(
                                        endPoint.x,
                                        heightMap.getHeight(
                                                new Vector3f(endPoint.x, 0, endPoint.z)),
                                        endPoint.z));
                    } else {
    ...
    /**
     * Creates the bot ball. And run the simulation.
     *
     * @return The bot ball.
     */
    @NotNull
    ...
    public Runnable createBotBall() {
        ...
        return () -> {
            ...
            RuleBasedBot ruleBasedBot =
                    new RuleBasedBot(
                            new Entity(botBall),
                            new Entity(entitiesManager.getEndFlag()),
                            heightMap,
                            Consts.TARGET_RADIUS,
                            scene);
    ...
    /**
     * Creates the AI bot ball. And run the simulation.
     *
    ...
    public Runnable createAiBotBall() {
        ...
        return () -> {
            ...
            Entity aiBotBall =
                    new Entity(
                            modelManager.getAiBotBallModel(),
                            new Vector3f(pathManager.getStartPoint()),
                            new Vector3f(0, 0, 0),
                            5);
